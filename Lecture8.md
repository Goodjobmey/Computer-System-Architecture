...
Например, если бы в формуле использовалось другое действие: `E = (A * B) + (C * D)` , то для хранения промежуточного результата понадобилась бы дополнительная ячейка памяти, так как аккумулятор смог бы удержать только одно промежуточное значение.
