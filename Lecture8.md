...
В данной модели память разделена на _память команд_ и _память данных_. Регистр счётчика команд (Program Counter, PC) указывает адрес текущей инструкции, при этом общее количество возможных инструкций составляет 256. Из памяти команд данные поступают в регистр инструкций (Instruction Register, IR), где хранится активная команда или операционный код.
Регистр адреса (Address Register, AR) содержит адрес памяти, который будет использован в следующих тактах.
...
Кроме того, предусмотрены два флага состояния **Zero** и **Carry** которые используются для корректной работы устройства управления, позволяя отслеживать результат обработки данных и реагировать соответствующим образом.
