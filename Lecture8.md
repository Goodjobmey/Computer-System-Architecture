...
Машина фон-Неймана является абстрактной моделью ЭВМ, так как многие детали в архитектуре этого вычислителя не был конкретизированы. Это сделано специально, чтобы не сковывать творческого подхода к делу у инженеров-разработчиков новых ЭВМ. Можно сказать, что машина фон Неймана описана не на внутреннем, а только на концептуальном уровне видения архитектуры. 

Так что давайте рассмотрим более реальную организацию архитекутры фон-Неймана:

![](https://gitlab.se.ifmo.ru/computer-systems/csa-rolling/-/raw/master/fig/von-neumann-example.png)

Здесь память разделена на _память команд_ и _память данных_. В память команд идет регистр Programm Counter (PC), который адресует текущую инструкцию (всего 256 инструкций). Память команд читается в Instruction Register (IR), в котором хранится текущая инструкция или операционный код. 
Adress Register (AR) -- регистр адреса, в котором хранится адрес памяти, который будет использоваться в следущих тактах. 
Здесь есть один мультиплексор (MUX2), который позволяет выбрать левый вход АЛУ, то есть либо прочитать данные из памяти данных, либо прочитаьь то, что пришло с памяти команд. Второй аргумент АЛУ приходит к нам исключительно из аккумулятора. В аккумулятор записывается сам результат. 
Также есть два флага Zerro и Carry, которые нужны для того, чтобы устройство управления могло понимать, что произошло с данными, и правильно регаировать на это. 
...
Основная идея заключается в том, что мы пытаемся управлять процессором, наиболее эффективным способом: давайте усложним систему команд так, чтобы для наших задач процессор делал как можно меньше ненужных операций. Так и появились CISC процессоры.
