...
mul R1 <-  R1 * B     ; 1 operand
add  R1 <- R1 + C
store Y<- R1          ;            (2)
```

В данной архитектуре аккумуляторный регистр перестает быть неявным и явно указывается в качестве аргумента команды. Рассматривается вычисление той же самой линейной функции, что и в аккумуляторной системе.

Из-за необходимости явного указания адресов регистров для чтения и записи размер машинного кода увеличивается: в каждой инструкции требуется передавать дополнительные адреса регистров. Однако в целом общая структура программы сохраняется.

Следует отметить, что в данном примере мы теряем в эффективности за счёт необходимости хранения и передачи адресов операндов вместе с инструкциями по шине. Вместе с тем, система с множественными регистрами даёт преимущества при реализации более сложных вычислений: отпадает необходимость явно загружать (load) и сохранять (store) промежуточные результаты в память, что упрощает и ускоряет выполнение программ.
